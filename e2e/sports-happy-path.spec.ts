...
test('Complete Sports Session Lifecycle', async ({ page }) => {
  ...
  // --- Robust cleanup at the start (match cardio test pattern) ---
  await page.evaluate(async () => {
    ...
    do {
      ...
      // Find all sports sessions for the test user
      ...
      // Clean up any orphaned calendar events with source = 'sports' and user_id
      await supabase.from('calendar_events').delete().eq('user_id', userId).eq('source', 'sports');
      cleanupIterations++;
    } while (true);
  });
  ...
  const activityType = 'Test Basketball';
  const location = 'Test Court';
  const performanceNotes = 'Test sports session for E2E testing';
  ...
  const sessionData = await page.evaluate(async () => {
    ...
    if (error) {
      console.error('[E2E] Error querying sports sessions:', error);
      return null;
    }
  ...
  const sessionCount = await page.evaluate(async () => {
    ...
    const { data: sportsSessions } = await supabase
      ...
      .select('id')
      .eq('user_id', userId)
      .eq('activity_type', 'Test Basketball');
  ...
  const sportsCard = page.locator('li, div').filter({ hasText: newActivityType }).first();
  ...
  // --- Robust cleanup at the end (match cardio test pattern) ---
  await page.evaluate(async () => {
    const supabase = window.supabase;
    const { data: session } = await supabase.auth.getSession();
    const userId = session.session.user.id;
  ...
  // Optionally, reload and confirm no test sports sessions remain
  await page.goto('http://localhost:3000/fitness/sports');
  await expect(page.getByRole('heading', { name: /sports/i, level: 1 })).toBeVisible();
  const testSportsLinks = await page.locator('a, li, div', { hasText: 'Test Basketball' }).count();
  if (testSportsLinks > 0) {
    console.log(`Warning: ${testSportsLinks} test sports sessions still present after cleanup, but continuing with test`);
  }

  console.log('[E2E] ✅ Complete sports session lifecycle test completed successfully');
}); 