...
export default function SportsSessionPage() {
  ...
  // Check if the requested sports ID matches the active session
  useEffect(() => {
    if (sportsId && activeSportsId && sportsId !== activeSportsId) {
      // If there's a mismatch, refresh the session to get the correct one
      refreshSports();
    }
  }, [sportsId, activeSportsId, refreshSports]);
        ...
        <form onSubmit={handleStartSports} className="space-y-4">
          <div>
            ...
            <FormInput
              ...
              required
              placeholder="e.g. Basketball, Soccer, Tennis, Hiking"
            />
          </div>
          <div>
  ...
  return (
    <div className="max-w-xl mx-auto p-6 mt-8 bg-panel rounded shadow">
      ...
      {sportsData.performance_notes && <p className="mb-2"><strong>Notes:</strong> {sportsData.performance_notes}</p>}
      ...
      <div className="text-sm text-muted-foreground">
        <p>Your sports session is active. Click &quot;End Sports Session&quot; when you&apos;re finished to save your session.</p>
      </div>
    </div>
  );
