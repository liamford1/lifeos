...
export default function ViewOrEditCardioSession({ params }) {
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 space-y-4">
      ...
      {isEditing ? (
        <>
          <h1 className="text-2xl font-bold">✏️ Edit Cardio Session</h1>
          <p className="text-gray-400">Update your cardio session details.</p>
          <CardioForm initialData={session} onSubmit={handleUpdate} />
        </>
      ) : (
        <>
          <h1 className="text-2xl font-bold">{session.activity_type}</h1>
          <p className="text-gray-400">View your cardio session details.</p>
